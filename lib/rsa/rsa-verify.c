...
int rsa_verify_key(const struct rsa_public_key *key, const uint8_t *sig,
...
static uint32_t *rsa_convert_big_endian(const uint32_t *src, int len)
{
	uint32_t *dst;
	int i;

	dst = calloc(sizeof(uint32_t), len);
	if (dst) {
		for (i = 0; i < len; i++)
		...
		const void *hash, uint8_t *sig, uint sig_len, int node)
{
	...
	key.modulus = rsa_convert_big_endian(modulus, key.len);
	key.rr = rsa_convert_big_endian(rr, key.len);
