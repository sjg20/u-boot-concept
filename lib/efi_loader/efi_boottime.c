 ...
 * @memoryp:     address of pointer to allocated memory
 *
						  ...
						  uint64_t *memoryp)
{
	...
	ptr = (void *)*memoryp;
	EFI_ENTRY("%d, %d, 0x%zx, %p", type, memory_type, pages, memoryp);
	r = efi_allocate_pages(type, memory_type, pages, &ptr);
	*memoryp = (ulong)ptr;
					      ...
					      efi_uintn_t pages)
{
	...
	r = efi_free_pages((void *)4memory, pages);
	return EFI_EXIT(r);
}
