...
struct pwm_ops {
	 ...
	 */
	int (*set_config)(struct udevice *dev, uint period_ns, uint duty_ns);
 ...
 * @return 0 if OK, -ve on error
 */
int pwm_set_config(struct udevice *dev, uint period_ns, uint duty_ns);
 ...
 * @return 0 if OK, -ve on error
 */
int pwm_set_enable(struct udevice *dev, bool enable);
