...
#ifndef EFI_DP_UTILS_H
#define EFI_DP_UTILS_H
 ...
 *
 * Construct a device path representing the active/default U-Boot Ethernet
 * device. The caller is responsible for freeing the allocated memory (e.g.,
 * using efi_free()).
 *
 * Return: Pointer to the new Ethernet device path, or NULL on allocation
 * failure or if no Ethernet device is available.
 */
