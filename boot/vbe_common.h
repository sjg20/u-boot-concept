 ...
 * @offset: Block offset to read from
 * @size: Size of data to read
 * @buf: Destination buffer for the data
 * Return: 0 if OK, -ve on error
 */
int vbe_read_nvdata(struct udevice *blk, ulong offset, ulong size, u8 *buf);

/**
 * vbe_read_version() - Read the version information from a block device
 *
 * @blk: Device to read from
 * @offset: Block offset to read from
 * @version: Destination buffer for the version data
 * @max_size: Size of the buffer at @version
 * Return: 0 if OK, -ve on error
 */
...
/**
 * vbe_get_blk() - Decode the storage-device string
 *
 * @storage: Storage-device string, e.g. mmc1
 * @blkp: Returns referenced block device, on success
 * Return: 0 if OK, -ve on error
 */
