			 ...
			 char **err_msgp)
{
	...
	*err_msgp = NULL;
	...
	node_inc = calloc(sizeof(char *), count);
	if (!node_inc) {
		*err_msgp = "Out of memory for include node list";
		return -1;
	}
	debug("Hash nodes (%d):\n", count);
	...
	fdt_regions = calloc(sizeof(*fdt_regions), max_regions);
	if (!fdt_regions) {
		*err_msgp = "Out of memory for fdt hash regions";
		return -1;
	}
	...
	region = fit_region_make_list(fit, fdt_regions, count);
	free(fdt_regions);

	if (info.algo->verify(&info, region, count, fit_value, fit_value_len)) {
		*err_msgp = "Verification failed";
		return -1;
	}
