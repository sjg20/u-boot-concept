...
static int omap2430_musb_of_to_plat(struct udevice *dev)
{
	...
	plat->musb_config.multipoint = fdtdec_get_int(fdt, node,
							  "multipoint",
							  -1);
	...
	plat->musb_config.num_eps = fdtdec_get_int(fdt, node,
						       "num-eps", -1);
	...
	plat->musb_config.ram_bits = fdtdec_get_int(fdt, node,
							"ram-bits", -1);
	...
	plat->plat.power = fdtdec_get_int(fdt, node,
								"power", -1);
	...
	plat->plat.mode = fdtdec_get_int(fdt, node,
										"mode", -1);
	if (plat->plat.mode < 0) {
