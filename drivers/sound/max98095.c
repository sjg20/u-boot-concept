			...
			int bits_per_sample)
{
	...
	ret = max98095_device_init(pcodec_info);
	if (ret < 0) {
		debug("%s: max98095 codec chip init failed\n", __func__);
		return ret;
	}

	ret = max98095_set_sysclk(pcodec_info, mclk_freq);
	if (ret < 0) {
		debug("%s: max98095 codec set sys clock failed\n", __func__);
		return ret;
	}

	ret = max98095_hw_params(pcodec_info, sampling_rate,
				bits_per_sample);

	if (ret == 0) {
		ret = max98095_set_fmt(pcodec_info,
					SND_SOC_DAIFMT_I2S |
