...
int max98090_do_init(struct maxim_codec_priv *pcodec_info,
			int sampling_rate, int mclk_freq,
			int bits_per_sample)
{
	int ret = 0;

	ret = max98090_device_init(pcodec_info);
	if (ret < 0) {
		debug("%s: max98095 codec chip init failed\n", __func__);
		return ret;
	}

	ret = max98090_set_sysclk(pcodec_info, mclk_freq);
	if (ret < 0) {
		debug("%s: max98095 codec set sys clock failed\n", __func__);
		return ret;
	}

	ret = max98090_hw_params(pcodec_info, sampling_rate,
				bits_per_sample);

	if (ret == 0) {
		ret = max98090_set_fmt(pcodec_info,
					SND_SOC_DAIFMT_I2S |
