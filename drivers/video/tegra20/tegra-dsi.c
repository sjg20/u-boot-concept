...
static void tegra_dsi_mipi_pad_calibrate(struct dsi_pad_ctrl_reg *pad)
{
	u32 value;
...
static void tegra_dsi_mipi_calibrate(struct tegra_dsi_priv *priv)
{
	...
	if (ret)
		log_err("failed to enable MIPI calibration: %d\n", ret);

	tegra_dsi_mipi_pad_calibrate(&priv->dsi->pad);

	ret = misc_write(priv->mipi, 0x60, NULL, 0);
	if (ret)
		log_err("failed to calibrate: %d\n", ret);
}
...
static int tegra_dsi_bridge_probe(struct udevice *dev)
{
	...
	if (priv->version) {
		ret = uclass_get_device_by_phandle(UCLASS_MISC, dev,
						   ...
						   &priv->mipi);
		if (ret) {
			log_err("cannot get MIPI: error %d\n", ret);
			return log_ret(ret);
		}
	}
