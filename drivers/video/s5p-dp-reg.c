...
void s5p_dp_init_analog_func(struct s5p_dp_device *dp)
{
	...
	reg = readl(&base->dp_debug_ctl);
	reg &= ~(F_PLL_LOCK | PLL_LOCK_CTRL);
	writel(reg, &base->dp_debug_ctl);
	...
	if (s5p_dp_get_pll_lock_status(dp) == PLL_UNLOCKED) {
		...
		while (s5p_dp_get_pll_lock_status(dp) == PLL_UNLOCKED) {
			...
			if (DP_TIMEOUT_LOOP_COUNT < timeout_loop) {
				debug("failed to get pll lock status\n");
				return;
			}
			udelay(10);
		}
...
void s5p_dp_init_aux(struct s5p_dp_device *dp)
{
	...
	reg = AUX_BIT_PERIOD_DELAY(3) | AUX_HW_RETRY_COUNT_SEL(0)|
		...
		u32 n_value)
{
	...
	if (type == REGISTER_M) {
		reg = readl(&base->sys_ctl_4);
		reg |= FIX_M_VID;
		writel(reg, &base->sys_ctl_4);
		reg = m_value & 0xff;
