...
{
	const int vref_to_sync = 1, href_to_sync = 1;
...
static int get_backlight_info(const void *blob, int *vddp, int *enablep,
			      int *pwmp)
{
	...
	*pwmp = 0;
	...
	if (backlight < 0)
		return -ENOENT;
	if (fdtdec_decode_gpio(blob, backlight, "enable-gpios", &gpio))
		return -ENOENT;
	*enablep = gpio.gpio;
	...
	*pwmp = fdt32_to_cpu(prop[1]);
	...
	if (power < 0)
		return -ENOENT;
	if (fdtdec_decode_gpio(blob, power, "gpio", &gpio))
		return -ENOENT;
	*vddp = gpio.gpio;
		 ...
		 struct display_timing *timing)
{
	...
	if (!get_backlight_info(blob, &vdd_gpio, &enable_gpio, &pwm)) {
		gpio_request(vdd_gpio, "vdd_gpio");
	...
	/* Configure dc mode */
	ret = update_display_mode(dc_ctlr, timing);
	if (ret) {
	...
	if (enable_gpio != -1) {
		gpio_request(enable_gpio, "backlight_en");
		gpio_direction_output(enable_gpio, 1);
		debug("%s: backlight enable setting gpio %08x to %d\n",
		      __func__, enable_gpio, 1);
	}
