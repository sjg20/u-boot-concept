...
void scsi_setup_read16(const struct blk_desc *desc, struct scsi_cmd *pccb,
		       lbaint_t start, unsigned long blocks)
{
		       ...
		       void *buffer)
{
	...
	do {
		...
		if (start > SCSI_LBA48_READ) {
			...
			scsi_setup_read16(desc, pccb, start, blocks);
			start += blocks;
			blks -= blocks;
		} else
#endif
