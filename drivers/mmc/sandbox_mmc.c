...
static int sandbox_mmc_probe(struct udevice *dev)
{
	...
	if (plat->fname) {
		...
		if (priv->size % SIZE_MULTIPLE)
			log_warning("%s: file size %x is truncated to a multiple of %lx (i.e. %lx)\n",
				    plat->fname, priv->size, SIZE_MULTIPLE,
				    (priv->csize + 1) * SIZE_MULTIPLE);
	} else {
		priv->csize = 0;
