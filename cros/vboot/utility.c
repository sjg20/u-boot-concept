...
#if 0
void VB2_DEBUG(const char *format, ...)
{
	va_list ap;
	va_start(ap, format);
	vprintf(format, ap);
...
VbError_t VbExBeep(uint32_t msec, uint32_t frequency)
{
	...
	if (sound_init(gd->fdt_blob)) {
		VB2_DEBUG("Failed to initialize sound.\n");
		return VBERROR_NO_SOUND;
	}

	VB2_DEBUG("About to beep for %d ms at %d Hz.\n", msec, frequency);
	if (!msec)
		return VBERROR_NO_BACKGROUND_SOUND;

	if (frequency) {
		if (sound_play(msec, frequency)) {
			VB2_DEBUG("Failed to play beep.\n");
			return VBERROR_NO_SOUND;
		}
	} else {
		VbExSleepMs(msec);
...
int board_should_enable_lcd_panel(const void *blob)
{
	 ...
	 * properly for now.
	 */
	node = cros_ofnode_config_node(blob);
	return node >= 0 &&
