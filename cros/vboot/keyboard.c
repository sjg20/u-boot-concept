...
{
	ofnode croscfg_node = cros_ofnode_config_node();

	if (!ofnode_valid(croscfg_node))
		return -ENOENT;

	if (!ofnode_read_u32_array(croscfg_node, "faft-key-remap-special",
	...
	if (!ofnode_read_u32_array(croscfg_node, "faft-key-remap-ascii",
