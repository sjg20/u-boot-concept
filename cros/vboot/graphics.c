				  ...
				  struct bitmap_file_header *file_header)
{
	...
	file_header->file_size = le32_to_cpu(fh->file_size);
	if (file_header->file_size != size) {
		LOG("Bitmap file size does not match cbfs file size\n");
		return CBGFX_ERROR_BITMAP_DATA;
	}
	file_header->bitmap_offset = le32_to_cpu(fh->bitmap_offset);
			...
			struct vector *dim_org)
{
	...
	header->header_size = le32_to_cpu(h->header_size);
	...
	header->bits_per_pixel = le16_to_cpu(h->bits_per_pixel);
