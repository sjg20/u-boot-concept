...
static int load_archive(const char *str, struct directory **dest)
{
	...
	if (ret) {
		vboot_log(LOGL_ERR, "Cannot read locale '%s'\n", str);
	...
	if (dir->size != size) {
		vboot_log(LOGL_ERR,
			  "archive size %x does not match region size %x\n",
			  dir->size, size);
		return VBERROR_INVALID_BMPFV;
	}
...
static VbError_t vboot_draw_footer(struct vboot_info *vboot, uint32_t locale)
{
...
static VbError_t draw_base_screen(struct vboot_info *vboot, uint32_t locale,
				  int show_language)
{
	...
	RETURN_ON_ERROR(vboot_draw_footer(vboot, locale));
...
static VbError_t vboot_draw_wait(struct vboot_info *vboot, struct params *p)
{
	 ...
	 */
	RETURN_ON_ERROR(vboot_draw_base_screen_without_language(vboot, p));
...
static VbError_t draw_ui(struct vboot_info *vboot, uint32_t screen_type,
			 struct params *p)
{
...
static int vboot_init_locale(struct vboot_info *vboot)
{
	...
	/* Parse the list */
	vboot_log(LOGL_INFO, "Supported locales:\n");
	loc = loc_start;
	while (loc - loc_start < size
		...
		if (!lang || !strlen(lang))
			break;
		vboot_log(LOGL_INFO, " %s,", lang);
		locale_data.codes[locale_data.count] = lang;
		locale_data.count++;
	}
	free(locales);

	vboot_log(LOGL_INFO, " (%d locales)\n", locale_data.count);
...
static VbError_t vboot_init_screen(struct vboot_info *vboot)
{
	...
	if (ret) {
		vboot_log(LOGL_ERR, "Cannot find video device (err=%d)\n", ret);
		return VBERROR_UNKNOWN;
	}

	ret = uclass_first_device_err(UCLASS_VIDEO_CONSOLE, &vboot->console);
	if (ret) {
		vboot_log(LOGL_ERR, "Cannot find console device (err=%d)\n",
			  ret);
		return VBERROR_UNKNOWN;
	}
	...
	if (ret) {
		vboot_log(LOGL_ERR, "cbgfx_init() failed (err=%d)\n", ret);
		return VBERROR_UNKNOWN;
	}
	...
	if (ret) {
		vboot_log(LOGL_ERR, "Failed to load locales (err=%d)\n", ret);
		return VBERROR_INVALID_BMPFV;
	}
...
static void update_backlight(struct vboot_info *vboot, bool enable)
{
	...
	ret = panel_set_backlight(vboot->panel, enable ? BACKLIGHT_DEFAULT :
				  BACKLIGHT_OFF);
	if (ret)
		vboot_log(LOGL_WARNING, "Failed to set backlight\n");
}
...
int vboot_draw_screen(uint32_t screen, uint32_t locale)
{
	...
	RETURN_ON_ERROR(draw_ui(vboot, screen, &p));
