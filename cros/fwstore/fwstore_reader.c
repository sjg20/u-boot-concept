...
void fwstore_reader_setup(struct udevice *dev, int offset, int size)
{
	...
	plat->size = size;
	plat->cur = 0;
	log(LOGC_VBOOT, LOGL_DEBUG, "'%s': setup, base_offset=%x, size=%x\n",
	    dev->name, plat->base_offset, plat->size);
}
...
int fwstore_reader_restrict(struct udevice *dev, int offset, int size)
{
	...
	plat->size = size;
	plat->cur = 0;
	log(LOGC_VBOOT, LOGL_DEBUG, "Restricting '%s' to offset=%x, size=%x\n",
	    dev->name, plat->base_offset, plat->size);
			       ...
			       int size)
{
	...
	log(LOGC_VBOOT, LOGL_DEBUG, "%s: pos %x, size=%x\n", dev->name, pos,
	    plat->size);
	...
	if (ret)
		return ret;
	plat->cur += size;
	log(LOGC_VBOOT, LOGL_DEBUG,
	    "%s: read %x at %x, offset=%x, size=%x, limit=%x\n", dev->name,
	    size, pos, plat->base_offset, plat->size,
