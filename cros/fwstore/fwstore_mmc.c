			   ...
			   uint32_t count)
{
	...
	if (offset < vboot_mmc->ro_section_size &&
		...
		VB2_DEBUG("Boot partition access not contiguous\n");
		return -EINVAL;
	}
...
static int read_mmc(fwstore_t *file, uint32_t offset, uint32_t count,
		    void *buf)
{
	...
	VB2_DEBUG("offset=%#x, count=%#x\n", offset, count);
	partition = check_partition(vboot_mmc, offset, count);
	if (partition < 0)
		return -1;

	if (partition == 2) {
		VB2_DEBUG("Reading from partition 2\n");
	...
	VB2_DEBUG("Reading %d blocks\n", total_blocks);

	tmp_buf = malloc(MMC_MAX_BLOCK_LEN*total_blocks);
	if (!tmp_buf) {
		VB2_DEBUG("Failed to allocate buffer\n");
		goto out;
	}
	...
	if (err) {
		VB2_DEBUG("Failed to open boot partition %d\n", partition);
		goto out_free;
	}
	...
	n = vboot_mmc->mmc->block_dev.block_read(vboot_mmc->device,
						 ...
						 tmp_buf);
	if (n != total_blocks) {
		VB2_DEBUG("Failed to read blocks\n");
		goto out_close;
	}
...
out_close:
	...
	if (err) {
		VB2_DEBUG("Failed to close boot partition\n");
		ret = -1;
	}
 ...
 */
static int write_mmc(fwstore_t *file, uint32_t offset, uint32_t count,
		void *buf)
{
	struct vboot_mmc *vboot_mmc = file->context;
	...
	if (offset % MMC_MAX_BLOCK_LEN) {
		VB2_DEBUG("Offset of %d bytes not aligned to 512 byte boundary\n",
			offset);
		return -1;
	}

	if (count % MMC_MAX_BLOCK_LEN) {
		VB2_DEBUG("Count of %d bytes not aligned to 512 byte boundary\n",
			count);
		return -1;
	}
	...
	if (partition == 2) {
		VB2_DEBUG("Writing to partition 2\n");
		offset -= vboot_mmc->ro_section_size;
	}
	...
	num = vboot_mmc->mmc->block_dev.block_write(vboot_mmc->device,
						    ...
						    buf);
	if (num != total_blocks) {
		VB2_DEBUG("Failed to write blocks\n");
		ret = -1;
		goto out;
	}
...
static int close_mmc(fwstore_t *file)
{
	free(file->context);
	return 0;
}

static int sw_wp_enabled_mmc(fwstore_t *file)
{
	struct vboot_mmc *vboot_mmc = file->context;
...
int fwstore_open(fwstore_t *file)
{
	...
	if (node < 0) {
		VB2_DEBUG("fail to look up phandle: %d\n", node);
		return -1;
	}

	parent = fdt_parent_offset(blob, node);
	if (parent < 0) {
		VB2_DEBUG("fail to look up MMC parent: %d\n", parent);
		return -1;
	}

	mmc = mmc_get_device_by_node(blob, parent);
	if (!mmc) {
		VB2_DEBUG("fail to find MMC for node %d\n", parent);
		return -1;
	}

	err = mmc_init(mmc);
	if (err) {
		VB2_DEBUG("fail to initialize MMC: error %d\n", err);
		return -1;
	}
	...
	if (node < 0) {
		VB2_DEBUG("fail to lookup ro section\n");
		return -1;
	}

	err = fdtdec_read_fmap_entry(blob, node, "wp-ro", &entry);
	if (err) {
		VB2_DEBUG("fail to determine ro section size\n");
		return -1;
	}

	vboot_mmc = malloc(sizeof(*vboot_mmc));
	if (!vboot_mmc) {
		VB2_DEBUG("fail to allocate context structure\n");
		return -1;
	}
