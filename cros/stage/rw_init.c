...
static int gbb_init(struct vboot_info *vboot)
{
	...
	if (ret)
		return ret;
	vboot_log(LOGL_INFO, "The GBB signature is at %p and is: ",
		  hdr->signature);
	for (int i = 0; i < GBB_SIGNATURE_SIZE; i++)
		vboot_log(LOGL_INFO, " %02x", hdr->signature[i]);
	vboot_log(LOGL_INFO, "\n");
...
static int common_params_init(struct vboot_info *vboot, bool clear_shared_data)
{
	...
	vboot_log(LOGL_INFO, "Found shared_data_blob at %x, size %d\n",
				     ...
				     struct memory_wipe *wipe)
{
	...
	if (cros_ofnode_memory("/ramoops", &ramoops))
		VB2_DEBUG("RAMOOPS not contained within FDT\n");
	else
	...
	if (cros_ofnode_memory("/lp0", &lp0))
		VB2_DEBUG("LP0 not contained within FDT\n");
	else
...
static void wipe_unused_memory(struct vboot_info *vboot)
{
	...
	memory_wipe_sub(&wipe, get_current_sp() - MEMORY_WIPE_STACK_MARGIN,
			gd->ram_top);

	/* Exclude the shared data between bootstub and main firmware. */
	memory_wipe_sub(&wipe, (uintptr_t)vboot->handoff,
			(uintptr_t)vboot->handoff +
				sizeof(struct vboot_handoff));

	memory_wipe_execute(&wipe);
}

static int vboot_do_init_out_flags(struct vboot_info *vboot, uint32_t out_flags)
{
...
int vboot_rw_init(struct vboot_info *vboot)
{
	...
	ret = cros_nvdata_read_walk(CROS_NV_SECDATA, ctx->nvdata,
				    sizeof(ctx->nvdata));
	if (ret == -ENOENT)
		; // TODO(sjg@chromium.org): vb2api_secdatak_create(ctx);
// 	else
// 		return ret;
	static u8 data[] = {
		0x70, 0x00, 0x03, 0x00, 0x00, 0x00, 0x0a, 0x37,
