		...
		const char *comment, int require_keys)
{
	...
	if (ret) {
		printf("Can't write signature for "
			"'%s' signature node in '%s' image node: %s\n",
	...
	for (iname = prop; iname < end; iname += strlen(iname) + 1) {
		...
		if (!hash_count) {
			printf("Failed to find any hash nodes in configuration"
				" '%s/%s' image '%s' - without these it is"
				" not possible to verify this image\n",
...
err_path:
	printf("Failed to get path for image '%s' in"
		...
		char **region_propp, int *region_proplen)
{
	...
	if (!region_prop) {
		printf("Out of memory setting up regions for"
			" configuration '%s/%s'\n", conf_name, sig_name);
		return -ENOMEM;
	}
