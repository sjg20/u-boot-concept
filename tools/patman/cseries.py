...
class Cseries(cser_helper.CseriesHelper):
    ...
    def archive(self, series):
        ...
        # Create the tags
        repo = pygit2.init_repository(self.gitdir)
        for version, (idnum, name, tag_name) in tag_info.items():
            commit = repo.revparse_single(name)
            repo.create_tag(tag_name, commit.hex, pygit2.GIT_OBJ_COMMIT,
    ...
    def _build_col(self, state, prefix='', base_str=None):
        ...
        if state == 'accepted':
            ...
            col = self.col.WHITE
        out = base_str or cser_helper.SHORTEN_STATE.get(state, state)
        pad = ' ' * (10 - len(out))
