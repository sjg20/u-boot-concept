...
def get_type_uuid(fip_type_or_uuid):
    ...
    if isinstance(fip_type_or_uuid, str):
        ...
        if not lookup:
            raise ValueError("Unknown FIP entry type '%s'" % fip_type)
        uuid = lookup.uuid
    else:
        fip_type = None
...
class FipReader(object):
    ...
    def get_entry(self, fip_type_or_uuid):
        """get_entry() - Find an etry by type or UUID
        ...
        raise ValueError("Cannot find FIP entry '%s'" % label)
