...
#define IFDTOOL_VERSION "1.1-uboot"
...
// flash descriptor
struct __attribute__((packed)) fdbar_t {
	uint32_t flvalsig;
	uint32_t flmap0;
	uint32_t flmap1;
	uint32_t flmap2;
...
// regions
struct __attribute__((packed)) frba_t {
	uint32_t flreg[5];
};

// component section
struct __attribute__((packed)) fcba_t {
	uint32_t flcomp;
	uint32_t flill;
	uint32_t flpb;
};
...
// pch strap
struct __attribute__((packed)) fpsba_t {
	uint32_t pchstrp[MAX_STRAPS];
} ;

// master
struct __attribute__((packed)) fmba_t {
	uint32_t flmstr1;
	uint32_t flmstr2;
	uint32_t flmstr3;
};
...
// processor strap
struct __attribute__((packed)) fmsba_t {
	uint32_t data[8];
};
