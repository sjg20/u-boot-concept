...
int upl_get_test_data(struct unit_test_state *uts, struct upl *upl)
{
	...
	region.base = 0x10;
	region.size = 0x20;
	ut_assertnonnull(alist_add(&mem.region, region, struct memregion));
	region.base = 0x30;
	region.size = 0x40;
	ut_assertnonnull(alist_add(&mem.region, region, struct memregion));
	region.base = 0x50;
	...
	region.base = 0x11;
	region.size = 0x12;
	ut_assertnonnull(alist_add(&memmap.region, region, struct memregion));
	region.base = 0x13;
	region.size = 0x14;
	ut_assertnonnull(alist_add(&memmap.region, region, struct memregion));
	region.base = 0x15;
	...
	region.base = 0x2b;
	region.size = 0x2c;
	ut_assertnonnull(alist_add(&memres.region, region, struct memregion));
	...
	region.base = 0xf1de0000;
	region.size = 0x100;
	ut_assertnonnull(alist_add(&upl->serial.reg, region, struct memregion));
	...
	region.base = 0xd0000000;
	region.size = 0x10000000;
	ut_assertnonnull(alist_add(&upl->graphics.reg, region, struct memregion));
...
static int dm_test_upl_info_norun(struct unit_test_state *uts)
{
	...
	ut_asserteq(1, upl->image.count);

	img = alist_get(&upl->image, 0, struct upl_image);
	ut_asserteq_str("firmware-1", fdt_get_name(fit, img->offset, NULL));
	ut_asserteq(CONFIG_TEXT_BASE, img->load);

	return 0;
}
UPL_TEST(dm_test_upl_info_norun, UT_TESTF_CONSOLE_REC | UT_TESTF_MANUAL);
