...
class TestBuildApi(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create a temp file path for symbols.def that tests can write to
        self.sympath = os.path.join(self.tmpdir.name, 'symbols.def')
...
'''
    ...
    def test_symbols_def_parser(self):
        """Test parsing symbol definition file format"""
        content = '''# Test symbols.def file
file: stdio.h
 printf
 scanf
...
'''
    ...
    def test_symbols_def_with_real_file(self):
        """Test parsing with realistic symbols.def file"""
...
file: stdlib.h
 ...
 calloc
'''
        symbols_path = self.write_tmp(symbols_content, 'realistic_symbols.def')
...
'''
 ...
 printf
'''
        symbols_def_path = self.write_tmp(content, 'redefine_symbols.def')
...
'''
        ...
        # Test the parser
        parser = RenameSymsParser(symbols_def_path)
        renames = parser.parse()
        ...
        outfiles = SymbolRedefiner.apply_renames([obj_file_path], renames,
                                                self.tmpdir.name)
        assert outfiles
...
'''
 ...
 custom_func
'''
        symbols_def_path = self.write_tmp(content, 'coverage_symbols.def')
...
'''
        ...
        with contextlib.redirect_stdout(captured_stdout):
            outfiles = SymbolRedefiner.apply_renames(
                [obj_file_defined, obj_file_no_symbols], renames,
                self.tmpdir.name, verbose=True)

        assert outfiles
        assert len(outfiles) == 2
        output = captured_stdout.getvalue()
        assert 'Copied and modified' in output
...
'''
        ...
        empty_syms = []
        obj_files = [obj_file]
        result = SymbolRedefiner.apply_renames(obj_files, empty_syms,
                                              self.tmpdir.name)

        # Should return the original obj_files unchanged
        assert result == obj_files
    ...
    def test_parse_args_errors(self):
        ...
        test_args = ['test.def', '--redefine', 'test.o']
        ...
        assert result == 1
        error_output = captured_stderr.getvalue()
        assert 'Error: --include-dir is required with --api' in error_output
 ...
 printf
'''
        symbols_def_path = self.write_tmp(content, 'main_test.def')
...
'''
        ...
        # Test redefine path
        test_args = [symbols_def_path, '--redefine', obj_file, '--output-dir',
                    self.tmpdir.name]
        captured_stdout = StringIO()
        with contextlib.redirect_stdout(captured_stdout):
            result = main(test_args)
...
'''
        ...
        test_args = [symbols_def_path, '--api', api_file, '--include-dir',
        ...
        assert result == 0
        output = captured_stdout.getvalue()
        assert 'Generated API header:' in output
 ...
 missing_function
'''
        symbols_def_path = self.write_tmp(content, 'failing_api.def')
        ...
        assert result == 1
        error_output = captured_stderr.getvalue()
        assert 'Missing header files:' in error_output
...
'''
        ...
        assert 'Missing header files:' in output
        assert 'nonexistent.h' in output
...
'''
        ...
        output = captured.getvalue()
        assert 'Missing function declarations:' in output
        assert 'nonexistent_function in stdio.h' in output
...
file: string.h
 strlen
'''
        symbols_def_path = self.write_tmp(content, 'test_symbols.def')

        # Mock sys.argv to simulate command line arguments
        original_argv = sys.argv
        try:
            sys.argv = ['build_api.py', symbols_def_path, '--dump']
            ...
            assert 'strlen' in output
            assert 'stdio.h' in output
            assert 'string.h' in output
