...
def setup_image(cons, devnum, part_type, img_size, second_part=False,
                basename='mmc'):
    ...
    spec = f'type={part_type:x}, size={img_size - 3}M, start=1M, bootable'
    if second_part:
        spec += '\ntype=c'
...
'''
    ...
    u_boot_utils.run_and_log(cons, f'rm -rf {scratch_dir}')
...
label Fedora-Workstation-armhfp-31-1.9 (5.3.7-301.fc31.armv7hl)
    ...
    u_boot_utils.run_and_log(
        cons, f'mkimage -f auto -d {inf} {os.path.join(scratch_dir, vmlinux)}')

    with open(os.path.join(scratch_dir, initrd), 'w', encoding='ascii') as fd:
        print('initrd', file=fd)

    mkdir_cond(os.path.join(scratch_dir, dtbdir))

    dtb_file = os.path.join(scratch_dir, f'{dtbdir}/sandbox.dtb')
    u_boot_utils.run_and_log(
        cons, f'dtc -o {dtb_file}', stdin=b'/dts-v1/; / {};')

    fsfile = 'vfat17M.img'
    u_boot_utils.run_and_log(cons, f'fallocate -l 17M {fsfile}')
...
def setup_efi_image(cons):
    ...
    devnum = 1
    basename = 'flash'
    fname = setup_image(cons, devnum, 0xc, 20, second_part=True,
                        basename=basename)

    scratch_dir = os.path.join(cons.config.persistent_data_dir, 'scratch')
    mkdir_cond(scratch_dir)

    efi_dir = os.path.join(scratch_dir, 'EFI')
    mkdir_cond(efi_dir)
    ...
    u_boot_utils.run_and_log(cons, f'fallocate -l 17M {fsfile}')
    u_boot_utils.run_and_log(cons, f'mkfs.vfat {fsfile}')
    u_boot_utils.run_and_log(cons, ['sh', '-c', f'mcopy -vs -i {fsfile} {scratch_dir}/* ::/'])
    u_boot_utils.run_and_log(cons, f'dd if={fsfile} of={fname} bs=1M seek=1')
    u_boot_utils.run_and_log(cons, f'rm -rf {scratch_dir}')
    u_boot_utils.run_and_log(cons, f'rm -f {fsfile}')
