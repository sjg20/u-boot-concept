					...
					struct rk322x_sdram_params *sdram_params)
{
	...
	if (dramtype == DDR3) {
		...
		send_command(pctl, 3, MRS_CMD,
			     ...
			     ((sdram_params->phy_timing.mr[0]  | DDR3_DLL_RESET) &
		...
		send_command(pctl, 3, MRS_CMD,
			     ...
			     (sdram_params->phy_timing.mr[1] & LPDDR23_OP_MASK) <<
			     LPDDR23_OP_SHIFT);
		send_command(pctl, 3, MRS_CMD,
			     (2 & LPDDR23_MA_MASK) << LPDDR23_MA_SHIFT |
			     (sdram_params->phy_timing.mr[2] & LPDDR23_OP_MASK) <<
			     LPDDR23_OP_SHIFT);
		send_command(pctl, 3, MRS_CMD,
			     (3 & LPDDR23_MA_MASK) << LPDDR23_MA_SHIFT |
			     (sdram_params->phy_timing.mr[3] & LPDDR23_OP_MASK) <<
			     LPDDR23_OP_SHIFT);
		if (dramtype == LPDDR3)
			send_command(pctl, 3, MRS_CMD,
					 (11 & LPDDR23_MA_MASK) << LPDDR23_MA_SHIFT |
					 (sdram_params->phy_timing.mr11 & LPDDR23_OP_MASK) <<
					 LPDDR23_OP_SHIFT);
	}
}

static void data_training(struct chan_info *chan)
{
	...
	while ((readl(&ddr_phy->ddrphy_reg[0xff]) & bw) != bw)
		rockchip_udelay(1);
					...
					struct rk322x_sdram_params *sdram_params)
{
	...
	writel(noc_timing->ddrtiming.d32, &axi_bus->ddrtiming);
	writel(noc_timing->ddrmode.d32, &axi_bus->ddrmode);
	writel(noc_timing->readlatency, &axi_bus->readlatency);
	writel(noc_timing->activate.d32, &axi_bus->activate);
	writel(noc_timing->devtodev.d32, &axi_bus->devtodev);
...
static void sdram_all_config(struct dram_info *dram,
				struct rk322x_sdram_params *sdram_params)
{
	u32 os_reg = 0;
	u32 cs1_row = 0;
	...
	printf("%s bw %d\n", __func__, config->bw);
	os_reg = dram_type << DDR_TYPE_SHIFT |
			0 << DDR_CHN_CNT_SHIFT |
			(config->rank - 1) << DDR_RANK_CNT_SHIFT |
			(config->col - 1) << DDR_COL_SHIFT |
			(config->bk == 3 ? 0 : 1) << DDR_BANK_SHIFT |
			(config->cs0_row - 13) << DDR_CS0_ROW_SHIFT |
			cs1_row << DDR_CS1_ROW_SHIFT |
			(2 >> config->bw) << DDR_BW_SHIFT |
			(2 >> config->dbw) << DDR_DIE_BW_SHIFT;
	writel(os_reg, &dram->grf->os_reg[2]);
}
...
{	
	...
	printf("ddr clk dpll\n");
	ret = clk_set_rate(&dram->ddr_clk, sdram_params->base.ddr_freq * MHz*2);
	printf("ret=%d\n", ret);
	if (ret < 0) {
	...
	sdram_all_config(dram, sdram_params);
...
static int rk322x_dmc_ofdata_to_platdata(struct udevice *dev)
{
	...
	ret = fdtdec_get_byte_array(blob, node, "rockchip,sdram-channel",
				    (u8 *)&params->ch[0],
				    sizeof(params->ch[0]));
	if (ret) {
		printf("%s: Cannot read rockchip,sdram-channel\n", __func__);
		return -EINVAL;
	}
...
size_t sdram_size_mb(struct dram_info *dram)
{
	...
	size_t size_mb = 0;
	u32 sys_reg = readl(&dram->grf->os_reg[2]);

	printf("%s %x\n", __func__, sys_reg);
	rank = 1 + (sys_reg >> DDR_RANK_CNT_SHIFT &
		DDR_RANK_CNT_MASK);
	col = 9 + (sys_reg >> DDR_COL_SHIFT & DDR_COL_MASK);
	bk = 3 - ((sys_reg >> DDR_BANK_SHIFT) & DDR_BANK_MASK);
	cs0_row = 13 + (sys_reg >> DDR_CS0_ROW_SHIFT &
			DDR_CS0_ROW_MASK);
	cs1_row = 13 + (sys_reg >> DDR_CS1_ROW_SHIFT &
			DDR_CS1_ROW_MASK);
	bw = 2 >> (((sys_reg >> DDR_BW_SHIFT)) &
		DDR_BW_MASK);
	row_3_4 = (sys_reg >> DDR_CHA_ROW_3_4_SHIFT) &
		DDR_CHA_ROW_3_4_MASK;
	chipsize_mb = (1 << (cs0_row + col + bk + bw - 20));

	printk("%s rank %d col %d bk %d cs0_row %d cs1_row %d bw %d row_3_4 %d\n",
			__func__, rank, col, bk, cs0_row, cs1_row, bw, row_3_4);
	if (rank > 1)
