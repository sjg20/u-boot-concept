...
static inline void gtt_write_powermeter(void *bar,
					const struct gt_powermeter *pm)
{
	for (; pm && pm->reg; pm++)
...
int gma_pm_init_post_vbios(pci_dev_t dev)
{
	...
	struct northbridge_intel_sandybridge_config sconf = {
		...
		.gpu_panel_port_select = 1,			/* eDP_A */
		.gpu_panel_power_cycle_delay = 6,		/*  500ms */
		.gpu_panel_power_up_delay = 2000,		/*  200ms */
		.gpu_panel_power_down_delay = 500,		/*  50ms */
		.gpu_panel_power_backlight_on_delay = 2000,	/*  200ms */
		.gpu_panel_power_backlight_off_delay = 2000,	/*  200ms */

		/* Set backlight PWM values for eDP */
		.gpu_cpu_backlight = 0x00000200,
		.gpu_pch_backlight = 0x04000000,
	};
	struct northbridge_intel_sandybridge_config *conf = &sconf;
	u32 reg32;
...
static void gma_read_resources(pci_dev_t dev)
{
	...
	if (res == NULL) {
		debug("gma: memory resource not found.\n");
		return;
	}
