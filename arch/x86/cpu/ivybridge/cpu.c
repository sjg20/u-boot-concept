...
static int pch_enable_lpc(void)
{
	...
	if (node < 0)
		return -ENOENT;
	if (fdtdec_get_int_array(blob, node, "gen-dec", values, 4))
		return -EINVAL;
	...
	pci_write_config32(PCH_LPC_DEV, LPC_GEN1_DEC, values[0]);
	pci_write_config32(PCH_LPC_DEV, LPC_GEN2_DEC, values[1]);
	pci_write_config32(PCH_LPC_DEV, LPC_GEN3_DEC, values[2]);
	pci_write_config32(PCH_LPC_DEV, LPC_GEN4_DEC, values[3]);
...
// 	enable_rom_caching();
	...
	debug("Back from sandybridge_early_initialization()\n");
