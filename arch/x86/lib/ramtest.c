...
static int ram_bitset_nodie(unsigned long start)
{
	...
	debug("DRAM bitset write: 0x%08lx\n", start);
	...
	debug("DRAM bitset verify: 0x%08lx\n", start);
	failures = 0;
	for (idx=0; idx<0x400; idx+=4) {
		test_pattern(idx, &addr, &value);
		...
		failures |= failed;
		if  (failed && !verbose) {
			printf("0x%08lx wr: 0x%08lx rd: 0x%08lx FAIL\n",
				 start + addr, value, value2);
		}
		if (verbose) {
			if ((addr & 0x0f) == 0)
				debug("%08lx wr: %08lx rd:",
					start + addr, value);
			if (failed)
				debug(" %08lx!", value2);
			else
