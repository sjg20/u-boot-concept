...
/*
 * for restricted C runtime env
 * Only used in env_get_f(). See cmd/nvedit.c
 */
...
int env_init(void)
{
	struct env_context *init_ctx, *ctx;
	init_ctx = ll_entry_get(struct env_context, uboot, env_contexts);
...
void env_relocate(void)
{
	...
	/*
	 * U-Boot environment is the only context that must be
	 * handled in special manner because it is allocated dynamically
	 * in env_init() for some initialization.
	 */
	if (ctx->has_inited)
	...
	if (ctx->set_addr)
		ctx->set_addr += gd->reloc_off;
	if (ctx->get_addr)
		ctx->get_addr += gd->reloc_off;

	ctx->htab = (struct hsearch_data *)
			((unsigned long)ctx->htab + gd->reloc_off);
	ctx->htab->ctx = ctx;
	ctx_uboot = ctx;
